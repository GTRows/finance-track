...
class FireServiceTest {
    ...
    void progressRatioCapsAtFour() {
        ...
        FireResponse res = service.compute(userId, null, null, null, null,
    ...
    void monthsToFiIsNullWhenContributionZeroAndReturnZero() {
        ...
        FireResponse res = service.compute(userId, null, BigDecimal.ZERO, BigDecimal.ZERO,
    ...
    void monthsToFiIsPositiveForReachableTarget() {
        ...
        FireResponse res = service.compute(userId, null, null,
                new BigDecimal("20000"), null, new BigDecimal("500000"));
    ...
    void expenseOverrideReplacesAverageExpenseAndMonthlyContributionRespectsIt() {
        ...
        FireResponse res = service.compute(userId, null, null, null,
    ...
    void netWorthSumsHoldingValuesWhenOverrideNull() {
        Portfolio p = Portfolio.builder().id(UUID.randomUUID()).userId(userId)
                .name("P").active(true).build();
        ...
        PortfolioHolding hBtc = PortfolioHolding.builder()
                .id(UUID.randomUUID()).portfolioId(p.getId()).assetId(btc.getId())
                .quantity(new BigDecimal("2")).build();
        PortfolioHolding hEth = PortfolioHolding.builder()
                .id(UUID.randomUUID()).portfolioId(p.getId()).assetId(eth.getId())
    ...
    void netWorthSkipsHoldingsWithMissingAssetOrPrice() {
        ...
        Asset btc = Asset.builder().id(UUID.randomUUID()).symbol("BTC").name("BTC")
                .assetType(AssetType.CRYPTO).currency("TRY")
                .price(new BigDecimal("100")).build();
        Asset noPrice = Asset.builder().id(UUID.randomUUID()).symbol("NP").name("NP")
                .assetType(AssetType.CRYPTO).currency("TRY").price(null).build();
        PortfolioHolding good = PortfolioHolding.builder()
                .id(UUID.randomUUID()).portfolioId(p.getId()).assetId(btc.getId())
                .quantity(new BigDecimal("1")).build();
        PortfolioHolding priceless = PortfolioHolding.builder()
                .id(UUID.randomUUID()).portfolioId(p.getId()).assetId(noPrice.getId())
                .quantity(new BigDecimal("5")).build();
        PortfolioHolding orphan = PortfolioHolding.builder()
                .id(UUID.randomUUID()).portfolioId(p.getId()).assetId(UUID.randomUUID())
        ...
        when(holdingRepo.findByPortfolioId(p.getId()))
                .thenReturn(List.of(good, priceless, orphan));
