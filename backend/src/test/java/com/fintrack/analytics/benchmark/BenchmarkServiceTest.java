...
class BenchmarkServiceTest {
    ...
    void fetchReturnsAllThreeBenchmarks() {
        when(yahoo.fetchHistory(eq("XU100.IS"), anyInt()))
                .thenReturn(List.of(point(Instant.parse("2026-04-01T00:00:00Z"), "10500")));
        when(yahoo.fetchHistory(eq("^GSPC"), anyInt()))
                .thenReturn(List.of(point(Instant.parse("2026-04-01T00:00:00Z"), "5200")));
        when(yahoo.fetchHistory(eq("GC=F"), anyInt()))
                .thenReturn(List.of(point(Instant.parse("2026-04-01T00:00:00Z"), "2350")));

        BenchmarkSeriesResponse res = service.fetch(90);

        assertThat(res.days()).isEqualTo(90);
        assertThat(res.series()).hasSize(3);
        assertThat(res.series())
                .extracting(BenchmarkSeries::code)
    ...
    void differentWindowSizesAreCachedSeparately() {
        when(yahoo.fetchHistory(eq("XU100.IS"), eq(90)))
                .thenReturn(List.of(point(Instant.parse("2026-04-01T00:00:00Z"), "100")));
        when(yahoo.fetchHistory(eq("XU100.IS"), eq(180)))
                .thenReturn(List.of(point(Instant.parse("2026-04-01T00:00:00Z"), "200")));
        when(yahoo.fetchHistory(eq("^GSPC"), anyInt())).thenReturn(List.of());
        when(yahoo.fetchHistory(eq("GC=F"), anyInt())).thenReturn(List.of());
    ...
    void keepsStaleCacheWhenSubsequentFetchReturnsEmpty() {
        ...
        // Invalidate by bypassing TTL via different window? Cache TTL is 1h so use a forced second
        // call
        // after time elapses. Here we just verify that empty response on fresh call does NOT
        // discard cache.
        // The current code writes to cache only on non-empty, so subsequent calls within TTL still
        // hit cache.
