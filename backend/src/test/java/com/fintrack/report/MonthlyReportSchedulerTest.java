...
class MonthlyReportSchedulerTest {
    ...
    void skipsWhenSchedulerDisabled() {
        ...
        verify(mailService, never())
                .sendHtmlWithAttachment(any(), any(), any(), any(), any(), any());
    }
    ...
    void skipsUnverifiedUsers() {
        ...
        when(userRepository.findAll())
                .thenReturn(
                        List.of(user("unverified", "u@x", false), user("verified", "v@x", true)));
    ...
    void sendsPreviousMonthAsPdfAttachment() {
        ...
        when(reportService.generateMonthlyBudgetPdf(eq(u.getId()), any(YearMonth.class)))
                .thenReturn(pdf);
    ...
    void continuesOnFailureForOneUser() {
        ...
        when(reportService.generateMonthlyBudgetPdf(eq(bad.getId()), any()))
                .thenThrow(new RuntimeException("boom"));
        when(reportService.generateMonthlyBudgetPdf(eq(good.getId()), any()))
