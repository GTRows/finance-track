...
/**
 * Response returned after successful authentication (login, register, refresh). When {@code
 * requiresTotp} is true the token fields are null; the client must submit the TOTP code along with
 * the {@code totpChallengeToken} via /auth/2fa/verify to complete the flow.
 */
