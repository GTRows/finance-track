...
public class SessionController {
    ...
    /**
     * Lists active sessions for the caller. The optional refreshToken (from the client's auth
     * store) is used to flag which row represents "this device".
     */
    @PostMapping("/list")
    public ResponseEntity<List<SessionResponse>> list(
        ...
        List<SessionResponse> sessions =
                refreshTokenService.listActive(user.getId()).stream()
                        .map(
                                rt ->
                                        new SessionResponse(
                                                rt.getId().toString(),
    ...
    public ResponseEntity<Void> revoke(
            @AuthenticationPrincipal FinTrackUserDetails user, @PathVariable("id") UUID id) {
        boolean revoked = refreshTokenService.revokeSession(user.getId(), id);
        if (!revoked) {
            throw new ResourceNotFoundException("Session not found");
        }
        auditService.success(
                AuditAction.SESSION_REVOKE, user.getId(), user.getUsername(), "session=" + id);
    ...
    public ResponseEntity<Map<String, Integer>> revokeOthers(
        ...
        auditService.success(
                AuditAction.SESSION_REVOKE_OTHERS,
                user.getId(),
                user.getUsername(),
                "count=" + removed);
