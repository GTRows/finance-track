...
/**
 * Rule-based auto-categorization. When a transaction is created without an explicit category, scan
 * its description against active rules for that txn type (lowercase substring match), ordered by
 * priority then age. First match wins; match_count increments so busy rules can be surfaced in the
 * UI.
 */
@Service
...
public class TransactionCategoryRuleService {
                        ...
                        r -> {
                            String[] info =
                                    catLookup.getOrDefault(
                                            r.getCategoryId(), new String[] {"Unknown", null});
    ...
    public CategoryRuleResponse create(UUID userId, UpsertCategoryRuleRequest req) {
        ...
        TransactionCategoryRule rule =
                TransactionCategoryRule.builder()
                        .userId(userId)
        ...
        log.info(
                "Category rule created: id={} pattern='{}' type={}",
                rule.getId(),
                rule.getPattern(),
                rule.getTxnType());
        ...
        Map<UUID, String[]> catLookup = buildCategoryLookup(userId);
        String[] info =
                catLookup.getOrDefault(rule.getCategoryId(), new String[] {"Unknown", null});
    ...
    /**
     * Find the first rule whose pattern (case-insensitive) appears in the description. Returns the
     * matched category id, or empty if no rule matched. On a hit, the rule's match_count is
     * incremented.
     */
    @Transactional
    public Optional<UUID> matchFor(
            UUID userId, BudgetTransaction.TxnType txnType, String description) {
        if (description == null || description.isBlank()) return Optional.empty();
        String haystack = description.toLowerCase(Locale.ROOT);

        List<TransactionCategoryRule> rules =
                ruleRepo.findByUserIdAndTxnTypeOrderByPriorityAscCreatedAtAsc(userId, txnType);
        for (TransactionCategoryRule rule : rules) {
            ...
            if (!needle.isBlank() && haystack.contains(needle)) {
                ...
                ruleRepo.save(rule);
                log.debug(
                        "Rule matched: id={} pattern='{}' -> category={}",
                        rule.getId(),
                        rule.getPattern(),
                        rule.getCategoryId());
    ...
    private void validateCategoryOwnership(
            UUID userId, UUID categoryId, BudgetTransaction.TxnType txnType) {
