...
public final class AllocationDtos {
    ...
    public record BucketResponse(UUID id, String name, BigDecimal percent, UUID categoryId, int ordinal) {
        public static BucketResponse from(AllocationBucket b) {
            return new BucketResponse(b.getId(), b.getName(), b.getPercent(), b.getCategoryId(), b.getOrdinal());
        }
    }

    public record PreviewRequest(
            @NotNull @PositiveOrZero BigDecimal income,
            @PositiveOrZero BigDecimal obligations
    ) {}

    public record AllocatedBucket(String name, UUID categoryId, BigDecimal percent, BigDecimal amount) {}
