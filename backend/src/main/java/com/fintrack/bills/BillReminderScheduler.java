...
/**
 * Daily pass that emails a digest of upcoming bill reminders per user. Each bill
 * is reminded at most once per reminder window via the {@code lastRemindedOn}
 * stamp, so a missed scheduler run still triggers the email the next day.
 */
@Component
...
public class BillReminderScheduler {
    ...
    public void checkReminders() {
        ...
        for (Map.Entry<UUID, List<PendingReminder>> entry : perUser.entrySet()) {
            ...
            try {
                mailService.sendHtml(user.getEmail(), buildSubject(entry.getValue()), buildBody(user, entry.getValue()));
    ...
    private String buildBody(User user, List<PendingReminder> pending) {
        ...
        for (PendingReminder pr : pending) {
            ...
            String when = pr.daysUntil == 0 ? "today" : pr.daysUntil + " day" + (pr.daysUntil == 1 ? "" : "s");
            rows.append("""
                    <tr>
