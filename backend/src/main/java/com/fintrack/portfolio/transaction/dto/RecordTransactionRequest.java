...
public record RecordTransactionRequest(
        ...
        @NotNull(message = "Transaction type is required") TxnType txnType,
        @NotNull(message = "Quantity is required")
                @DecimalMin(value = "0.00000001", message = "Quantity must be greater than zero")
                @Digits(
                        integer = 12,
                        fraction = 8,
                        message = "Quantity must have at most 12 integer and 8 fractional digits")
                BigDecimal quantity,
        @NotNull(message = "Unit price is required")
                ...
                @Digits(
                        integer = 16,
                        fraction = 4,
                        message = "Unit price must have at most 16 integer and 4 fractional digits")
                BigDecimal priceTry,
