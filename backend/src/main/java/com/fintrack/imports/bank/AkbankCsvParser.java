...
/**
 * Akbank current-account CSV export parser.
 *
 * <p>Format quirks: Windows-1254 encoding. Delimiter {@code ;}. Date {@code dd.MM.yyyy}. Decimal
 * {@code ,}. Sign convention: SINGLE column {@code Tutar} with {@code -} prefix for debit. Possible
 * 5-line metadata block before the header (account holder, IBAN, period, etc.) -- skipped until a
 * line containing {@code Tarih}. Footer "Toplam Borc" / "Toplam Alacak" rows -- ignored. The
 * metadata-block search is bounded at the first 10 lines.
 */
@Component
@Slf4j
public class AkbankCsvParser implements BankCsvParser {

    private static final DateTimeFormatter DATE = DateTimeFormatter.ofPattern("dd.MM.yyyy");
    private static final char DELIMITER = ';';
    private static final Charset CHARSET = Charset.forName("windows-1254");
    private static final int MAX_METADATA_LINES = 10;
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(input, charset))) {
            ...
            if (headerIdx < 0) {
                throw new IllegalArgumentException(
                        "Could not locate header row within first "
                                + MAX_METADATA_LINES
                                + " lines (expected 'Tarih')");
            }
    ...
    private static int findHeader(List<String> lines) {
        int limit = Math.min(MAX_METADATA_LINES, lines.size());
    ...
    private static boolean isFooter(String line) {
        ...
        return lower.startsWith("toplam");
    }
